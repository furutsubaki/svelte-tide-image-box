...
const onCatchDispatch = ({
    detail: image,
}: {
    detail: { image: TideImage };
}) => {
    // on:closeのみデータはありません
    console.log(image);
};
...
<main>
    ...
    <div>
        <h2>one image</h2>
        <p>
            `images`にTideImageを一つだけ入れた配列を指定することで、左右の切り替えUIが非表示になります
        </p>
    ...
    <div>
        <h2>multi image</h2>
        <p>
            `images`に複数のTideImageを入れることで左右に画像切り替えUIを表示します
        </p>
        <TideImageBox
            {images}
            on:open={onCatchDispatch}
            on:close={onCatchDispatch}
            on:change={onCatchDispatch}
        />
    </div>
    <div>
        <h2>use slot</h2>
        <p>
            default
            slotで画像の表示を親で制御できます。ただし`onClick`メソッドは必ず使用してください
        </p>
